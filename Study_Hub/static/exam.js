...
function submitExam() {
    ...
    // Save score to backend without blocking result rendering.
    const user = JSON.parse(localStorage.getItem('currentUser'));
    saveScoreToDatabase({
        email: user ? user.email : '',
        testName: document.getElementById('examTitle').textContent || 'CET Mock Test',
        canvaLink: new URLSearchParams(window.location.search).get('canvaLink') || window.location.href,
        totalQuestions: 60,
...
// Save score details in PostgreSQL via servlet endpoint.
function saveScoreToDatabase(payload) {
    if (!payload.email) {
        return;
    }
    ...
    params.append('testName', payload.testName);
    params.append('canvaLink', payload.canvaLink);
    params.append('totalQuestions', String(payload.totalQuestions));
    params.append('answeredQuestions', String(payload.answeredQuestions));
    params.append('correctAnswers', String(payload.correctAnswers));
    params.append('score', String(payload.score));
    ...
    fetch('SaveScoreServlet', {
        ...
        // Keep UI flow smooth even if score save fails.
    });
}
