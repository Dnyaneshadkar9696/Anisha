...
public class CreateMockTestServlet extends HttpServlet
 {
    ...
    throws ServletException, IOException 
    {
        ...
        try 
        {
            ...
            {
                sql = "SELECT subject_id, subject_name FROM subject WHERE course_id = ? ORDER BY subject_name";
                ps = con.prepareStatement(sql);
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)throws ServletException, IOException
    {
       try
       {
            ...
            String sql="insert into mocktest(test_name, duration, total_marks, subject_id, form_link) values(?,?,?,?,?)";
            ...
            ps.setString(5, form_link);
            int result = ps.executeUpdate();

            if(result>0)
            {
                response.sendRedirect("CreateMockTestServlet?course_id=" + courseId + "&msg=success");
            }
            else
            {
                response.sendRedirect("CreateMockTestServlet?course_id=" + courseId + "&msg=error");
            }
